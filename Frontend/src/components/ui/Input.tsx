...
const Input = forwardRef<HTMLInputElement, InputProps>(({
  ...
  const iconPadding = leftIcon ? 'pl-10' : rightIcon ? 'pr-10' : 'px-3';
  const sizeClasses = 'py-2.5 text-base';

  const widthClass = fullWidth ? 'w-full' : '';
  const classes = `${baseClasses} ${variantClasses} ${iconPadding} ${sizeClasses} ${widthClass} ${className}`.trim();
